@php($pending = auth()->check() && auth()->user()->roles->isEmpty())
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>@if($pending){{ __('Awaiting approval') }}@else{{ __('Forbidden') }}@endif</title>
    <style>
...
<body>
    <div class="card">
        @if($pending)
            <div class="icon">⏳</div>
            <h1>{{ __('Your account is awaiting approval') }}</h1>
            <p>{{ __('Thanks for registering! An administrator needs to grant you access before you can use the application.') }}</p>
            <p>{{ __('You will be able to sign in once your account has been approved.') }}</p>
        @else
            <div class="icon">🔒</div>
